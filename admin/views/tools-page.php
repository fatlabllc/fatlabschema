...
<div class="wrap fls-admin-page">
	...
	<div class="fls-tools-section">
		...
		$stats['with_schema'] = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*) FROM {$wpdb->postmeta} WHERE meta_key = %s AND meta_value = %s",
				'_fatlabschema_enabled',
				'1'
			)
		);
		...
		$most_used = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT meta_value FROM {$wpdb->postmeta} WHERE meta_key = %s GROUP BY meta_value ORDER BY COUNT(*) DESC LIMIT 1",
				'_fatlabschema_type'
			)
		);
	...
	<div class="fls-tools-section">
		...
		<button type="button" id="fls-clear-cache" class="button" data-confirm="<?php esc_attr_e( 'Clear all schema cache?', 'fatlabschema' ); ?>" data-success="<?php esc_attr_e( 'Cache cleared!', 'fatlabschema' ); ?>"><?php esc_html_e( 'Clear All Schema Cache', 'fatlabschema' ); ?></button>
	</div>
</div>
